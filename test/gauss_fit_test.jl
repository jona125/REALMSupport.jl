...
function gauss_test(tgt_peak, tgt_sigma, input_length, rand_mag; kwargs...)
    ...
    @test params[1] ≈ tgt_peak atol =
        100 * (rand_mag + sqrt(eps())) * tgt_peak / sqrt(2 * input_length + 1)
    @test params[2] ≈ tgt_sigma atol =
        100 * (rand_mag + sqrt(eps())) * tgt_sigma / sqrt(2 * input_length + 1)
end
