...
function coord_trans(img, angle, tilt, mask)
	(p, y, s) = size(img)
	img_trans = zeros(floor(Int,s*cos(tilt) + p*cos(angle))+12, y, floor(Int, s*sin(tilt) + p*sin(angle)))
	img_mask =  zeros(floor(Int,s*cos(tilt) + p*cos(angle))+12, y, floor(Int, s*sin(tilt) + p*sin(angle)))
	for i in 1:p
		for j in 1:s
