...
img_s = pipeline2(img_s;z_set=parse(Int64,ARGS[4]),z_angle=parse(Float64,ARGS[5]),x_angle = parse(Float64,ARGS[6]),y_angle=parse(Float64,ARGS[7]))
img_ = zeros(size(img_s)[1],size(img_s)[2],size(img)[4])
for i in 1:size(img)[4]
    img_s = img[:,:,:,i]
    
    img_s = imresize(img_s, new_size);
    @show("image transformation")
    img_re = pipeline2(img_s;z_set=parse(Int64,ARGS[4]),z_angle=parse(Float64,ARGS[5]),x_angle = parse(Float64,ARGS[6]),y_angle=parse(Float64,ARGS[7]))
    img_s = nothing
